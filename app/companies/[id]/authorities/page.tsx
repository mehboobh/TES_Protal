...
export default function AuthoritiesPage() {
  ...
  // Data States (All properly initialized as empty arrays)
  ...
  const handleSaveSafetyRecord = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    const formData = new FormData(e.currentTarget)
    setSafetyRecords([{
      id: generateId("SR"),
      ...
      status: formData.get("review") === "Unsatisfactory" || formData.get("review") === "Conditional" ? "warn" : "ok"
    }, ...safetyRecords])
    setShowAddSafety(false)
  }

  // Helper for KPI Dashboard
  const latestSafetyRecord = safetyRecords[0] || null;
  ...
  return (
    <div className="flex flex-col gap-6 pb-10 max-w-6xl">
      ...
      {/* 2. SAFETY RATING PERFORMANCE DASHBOARD */}
      <div className="grid lg:grid-cols-3 gap-6">
            ...
            {latestSafetyRecord ? (
              <>
                <div className="flex items-baseline gap-2">
                  <span className="text-5xl font-bold tracking-tighter text-foreground">{latestSafetyRecord.score}</span>
                </div>
                <div className="flex items-center gap-2 mt-3">
                  <Badge className={latestSafetyRecord.status === "ok" ? "bg-green-600 hover:bg-green-700 text-white" : "bg-yellow-600 hover:bg-yellow-700 text-white"}>
                    <ShieldCheck className="size-3 mr-1" /> {latestSafetyRecord.review}
                  </Badge>
              ...
              <div className="flex items-center h-full text-muted-foreground text-sm">
                No safety ratings logged yet.
              </div>
            )}
          </CardContent>
          <div className="p-4 border-t border-primary/10 text-xs text-muted-foreground flex justify-between bg-primary/[0.02]">
            <span>Last Audit: {latestSafetyRecord ? latestSafetyRecord.date : "N/A"}</span>
            <span className="font-semibold text-foreground">{latestSafetyRecord ? "Active Carrier" : "Pending"}</span>
          </div>
        </Card>
        ...
        <Card className="lg:col-span-2 shadow-sm">
          <CardHeader className="bg-muted/30 py-3 border-b flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-sm">Safety Rating Performance History</CardTitle>
              <CardDescription className="text-xs mt-1">Log updates to carrier profile scores.</CardDescription>
            </div>
            <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setShowAddSafety(!showAddSafety)}>
              {showAddSafety ? "Cancel" : <><Plus className="size-3 mr-1" /> Add Rating</>}
            </Button>
          </CardHeader>
          <CardContent className="p-0">
            {showAddSafety && (
              <form onSubmit={handleSaveSafetyRecord} className="p-4 bg-primary/5 border-b border-primary/20">
                <div className="grid sm:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input name="score" placeholder="e.g. 0.208" required />
                  </div>
                  <div className="space-y-2">
                    <Label>Review Status</Label>
                    <Select name="review" required defaultValue="Satisfactory">
                ...
                <div className="flex justify-end gap-2 mt-4">
                  <Button type="button" variant="outline" onClick={() => setShowAddSafety(false)}>Cancel</Button>
            ...
            <div className="divide-y text-sm">
              ...
              {safetyRecords.length === 0 ? (
                <div className="p-8 text-center text-muted-foreground text-sm">No safety records logged.</div>
              ) : (
                safetyRecords.map((sh) => (
                  <div key={sh.id} className="grid grid-cols-12 gap-4 p-3 items-center hover:bg-muted/20 transition-colors">
                    ...
                    <div className="col-span-3 text-right">
                      <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${sh.status === "ok" ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400" : "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400"}`}>
                        {sh.review}
                      </span>
                    </div>
                  </div>
          ...
          <CardContent className="p-0">
            {showAddCanadian && (
              <form onSubmit={handleSaveCanadian} className="p-6 bg-primary/5 border-b border-primary/20">
                ...
                <div className="mb-6">
                  <DocumentUploadZone isAutoFill={true} title="Mandatory: Safety Fitness Certificate / CVOR" description="Drop document here. OCR will extract NSC and Client numbers." />
                </div>
                <div className="grid sm:grid-cols-3 gap-4">
                  <div className="space-y-2"><Label>MVID / RIN (Provincial ID)</Label><Input name="mvid" placeholder="e.g. 0855-69101" required /></div>
                  <div className="space-y-2"><Label>CVOR / NSC Number</Label><Input name="cvor" placeholder="e.g. AB244-1590" required /></div>
            ...
            <div className="divide-y text-sm">
              ...
              {canadianRecords.length === 0 ? (
                <div className="p-8 text-center text-muted-foreground text-sm">No Canadian authorities on file. Click "Scan Safety Cert" or "Manual Add".</div>
              ) : (
        ...
        <Card className="shadow-sm">
          <CardHeader className="bg-muted/30 py-3 border-b flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={() => setShowAddUS(true)}><Plus className="size-4 mr-1" /> Manual Add</Button>
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {showAddUS && (
              <form onSubmit={handleSaveUS} className="p-6 bg-primary/5 border-b border-primary/20">
                ...
                <div className="grid sm:grid-cols-2 gap-4 mb-6">
                  <DocumentUploadZone isAutoFill={true} title="Mandatory: USDOT PIN Letter" description="OCR will extract USDOT and Portal PIN." />
                  <DocumentUploadZone isAutoFill={true} title="Mandatory: MC Authority Letter" description="OCR will extract the MC operating authority." />
                </div>
                <div className="grid sm:grid-cols-4 gap-4">
                  <div className="space-y-2"><Label>USDOT Number</Label><Input name="usdot" placeholder="e.g. 3303879" required /></div>
                  <div className="space-y-2"><Label>MC Number</Label><Input name="mc" placeholder="e.g. MC-1048291" required /></div>
            ...
            <div className="divide-y text-sm">
              ...
              {usRecords.length === 0 ? (
                <div className="p-8 text-center text-muted-foreground text-sm">No US authorities recorded. Click "Scan Letters" or "Manual Add".</div>
              ) : (
                usRecords.map((r) => (
                  <div key={r.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-muted/20 transition-colors">
                    <div className="col-span-3 font-mono text-xs">{r.id}</div>
        ...
        <Card className="shadow-sm">
          <CardHeader className="bg-muted/30 py-3 border-b flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={() => setShowAddUCR(true)}><Plus className="size-4 mr-1" /> Manual Add</Button>
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {showAddUCR && (
              <form onSubmit={handleSaveUCR} className="p-6 bg-primary/5 border-b border-primary/20">
                <div className="mb-6">
                  <DocumentUploadZone isAutoFill={true} title="Mandatory: UCR Certificate / Receipt" description="Upload the official UCR receipt for the current filing year." />
                </div>
        ...
        <Card className="border-destructive/30 shadow-sm">
          <CardHeader className="bg-destructive/5 py-3 border-b border-destructive/20 flex flex-row items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={() => setShowAddHazmat(true)}><Plus className="size-4 mr-1" /> Manual Add</Button>
            </div>
          </CardHeader>
          <CardContent className="p-0">
            {showAddHazmat && (
              <form onSubmit={handleSaveHazmat} className="p-6 bg-primary/5 border-b border-primary/20">
                <div className="mb-6">
                  <DocumentUploadZone isAutoFill={true} title="Mandatory: PHMSA License" description="Drop your PHMSA Registration Certificate here." />
                </div>
                <div className="grid sm:grid-cols-3 gap-4">
                  <div className="space-y-2"><Label>Registration Number</Label><Input name="registrationNo" placeholder="e.g. HM-10492" required /></div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="bg-muted/30 py-3 border-b flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription className="text-xs mt-1">Facility audits, DOT reviews, and safety compliance checks.</CardDescription>
          </div>
          <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => setShowAddAudit(!showAddAudit)}>
        ...
        <CardContent className="p-0">
          {showAddAudit && (
            <form onSubmit={handleSaveAudit} className="p-6 bg-primary/5 border-b border-primary/20">
              ...
              <div className="grid sm:grid-cols-3 gap-4">
                <div className="space-y-2"><Label>Audit / Intervention Type</Label><Input name="type" placeholder="e.g. NSC Facility Audit, DOT Review" required /></div>
                <div className="space-y-2"><Label>Date Completed / Due Date</Label><Input name="dueDate" type="date" required /></div>
                <div className="space-y-2">
                  <Label>Audit Outcome / Score</Label>
                  <Select name="score" required defaultValue="Pass">
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="flex justify-end gap-2 mt-4">
                <Button type="button" variant="outline" onClick={() => setShowAddAudit(false)}>Cancel</Button>
          ...
          <div className="divide-y text-sm">
            ...
            {audits.length === 0 ? (
                <div className="p-8 text-center text-muted-foreground text-sm">No audits or interventions on file.</div>
              ) : (
              audits.map((a) => (
                <div key={a.id} className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-muted/20 transition-colors">
                  ...
                  <div className="col-span-2 text-right">
                    <Badge variant="outline" className={`text-[11px] ${a.score === "Pass" ? "text-green-700 bg-green-50 border-green-200" : "text-yellow-700 bg-yellow-50 border-yellow-200"}`}>
                      {a.score}
                    </Badge>
                  </div>
                </div>
