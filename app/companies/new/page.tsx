...
export default function NewCompanyPage() {
  ...
  const handleFormChange = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const uniqueFields = [
      { key: 'accIrp', label: 'IRP Account' },
      { key: 'usdot', label: 'US DOT Number' },
      { key: 'mc', label: 'MC Number' },
      { key: 'businessNo', label: 'Business Number / EIN' },
      { key: 'incorpNo', label: 'Incorporation Number' },
      { key: 'gstHst', label: 'GST / HST Account' },
      { key: 'mvid', label: 'MVID / RIN' },
      { key: 'nsc', label: 'NSC / CVOR' },
      { key: 'accIfta', label: 'IFTA Account' },
      { key: 'accNyhut', label: 'NY HUT Account' },
      { key: 'accNm', label: 'NM WDT Account' },
      { key: 'accKyu', label: 'Kentucky KYU' },
      { key: 'accOr', label: 'Oregon Account' },
      { key: 'accCt', label: 'CT DRS Account' },
      { key: 'scac', label: 'SCAC' },
    ...
    for (const field of uniqueFields) {
      ...
      if (val && val.trim().length > 2) {
        ...
        if (conflict) {
          identifierConflict = `CRITICAL ERROR: The ${field.label} '${val}' is already registered to '${conflict.name}'. This must be globally unique.`
          break
        }
      }
    }
    
    setStrictIdentifierError(identifierConflict)
  }
