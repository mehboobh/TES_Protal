...
export function OwnershipTab({ companyId, store, vehicle, records, evidence, onStoreChange, onStartOCR, onAttachEvidence, pendingEvidenceId, clearPendingEvidence, setError, setNotice, FieldComponent, DividerComponent, SectionTitleComponent, EmptyStateComponent, StatusPillComponent, readCompanies, money, addMonthsISO }: OwnershipTabProps) {
  ...
  return <div className="space-y-3"><Card><SectionTitleComponent title="Ownership" description="Historical ownership relationships and supporting documents." action={<div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => setShowArchived((value) => !value)}>{showArchived ? "Hide Archived" : "Show History"}</Button><Button size="sm" onClick={() => { setEditing(null); setShowForm(true) }}><Plus className="mr-1.5 size-3.5" />Add Ownership</Button></div>} /></Card>
    ...
    {showForm ? <OwnershipForm companyId={companyId} FieldComponent={FieldComponent} DividerComponent={DividerComponent} readCompanies={readCompanies} addMonthsISO={addMonthsISO} vehicle={vehicle} initial={editing} store={store} pendingEvidenceId={pendingEvidenceId} onStartOCR={onStartOCR} onAttachEvidence={onAttachEvidence} clearPendingEvidence={clearPendingEvidence} onClose={() => setShowForm(false)} onStoreChange={onStoreChange} setError={setError} setNotice={setNotice} /> : null}
  </div>
}
...
function OwnershipForm({ companyId, vehicle, initial, store, pendingEvidenceId, onStartOCR, onAttachEvidence, clearPendingEvidence, onClose, onStoreChange, setError, setNotice, FieldComponent, DividerComponent, readCompanies, addMonthsISO }: { companyId: string; vehicle: VehicleRecord; initial: VehicleOwnershipRecord | null; store: VehicleStore; pendingEvidenceId: string | null; onStartOCR: (documentType: string) => void; onAttachEvidence: (documentType: string) => void; clearPendingEvidence: () => void; onClose: () => void; onStoreChange: (store: VehicleStore) => void; setError: (value: string | null) => void; setNotice: (value: string | null) => void; FieldComponent: OwnershipFieldComponent; DividerComponent: OwnershipDividerComponent; readCompanies: () => Company[]; addMonthsISO: (dateValue: string, monthsValue: string) => string }) {
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/50 p-4"><div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-xl border bg-card shadow-2xl"><div className="flex items-center justify-between border-b px-4 py-3"><div><h2 className="text-sm font-bold">{initial ? "Edit" : "Add"} Ownership Record</h2><p className="text-[11px] text-muted-foreground">OCR/source evidence belongs inside this workflow.</p></div><button onClick={onClose}><X className="size-4" /></button></div><div className="space-y-4 p-4">
    ...
    <div className="grid gap-3 md:grid-cols-2"><FieldComponent label="Purchase Date"><ISODateInput className={inputClass} value={purchaseDate} onValueChange={setPurchaseDate} /></FieldComponent><FieldComponent label="Purchase Price"><Input className={inputClass} value={purchasePrice} onChange={(e) => setPurchasePrice(e.target.value)} placeholder="0.00" /></FieldComponent></div>
    ...
    <DividerComponent /><div className="grid gap-3 md:grid-cols-2"><FieldComponent label="Ownership Start Date" required><ISODateInput className={inputClass} value={ownershipStartDate} onValueChange={setOwnershipStartDate} required /></FieldComponent><FieldComponent label="Ownership End Date"><ISODateInput className={inputClass} value={ownershipEndDate} onValueChange={setOwnershipEndDate} /></FieldComponent></div>
    ...
    <div className="flex items-center justify-between gap-3 rounded-lg border bg-muted/20 p-3"><div><p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Attached Evidence</p><p className="mt-1 text-xs">{evidenceIds.length ? `${evidenceIds.length} document(s) attached.` : "No document attached yet."}</p><p className="mt-1 text-[10px] text-muted-foreground">Attach keeps the information above unchanged. Upload / OCR is only for extracting fields.</p></div><Button type="button" variant="outline" size="sm" onClick={() => onAttachEvidence(requiredDocs[0])}><Upload className="mr-1.5 size-3.5" />Attach Evidence</Button></div>
